...
function randomizeStart() {
	...
	setTimeout(() => {document.getElementById('hint-button').classList.toggle('fade')}, 2000)
	...
	'assets/images/fox-g836dca1d9_1920.jpg'
	...
	function openHint() {
		document.getElementById('hint-window').classList.toggle('fade')
	}
	...
	if (re.test(answer.value)){
		...
		switch (true){
			case caseArray[0].test(answer.value): 
				printFeedback('That is the red fox, the most common fox.');
				nukeImages();
				document.getElementById('vulpes-vulpes').classList.remove('fade');
				break;
			...
			default:
				...
				printFeedback('That is not a recognized fox.') 
		}
